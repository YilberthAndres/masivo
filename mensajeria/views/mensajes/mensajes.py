...
from mensajeria.models import Destinatarios, Personas, Mensajeria, Conversaciones
import requests
...
def obtener_mensajes_find(request, recipiente_id):
    if request.method == "POST":
        ...
        registros = Mensajeria.objects.filter(
            recipiente_id=recipiente_id, created_at__gte=limite_tiempo
        )
        ...
        # Devolver la respuesta JSON
        response_data = {"resultados": resultados, "chat_view": cantidad_registros}
...
def send_message_media(request):
    ...
    try:
        ...
        payload = {
            ...
            + tipo
            + "": {
                "link": "https://86bd-186-86-181-231.ngrok-free.app/static/temp/"
                + nombre_completo
            },
        }

        link_img = (
            "https://86bd-186-86-181-231.ngrok-free.app/static/temp/" + nombre_completo
        )
        response = requests.post(url, headers=headers, json=payload)
...
def messages_read(request, recipiente_id):
    try:
        Mensajeria.objects.filter(
            destinatario_id__isnull=True, recipiente_id=recipiente_id
        ).update(estado_id=746)
...
def find_archivar(request):
    try:
        ...
        try:
            ...
            response_data = {
                "success": False,
                "message": "Ocurrio un error al actualizar conversacion.",
            }
